...
- AI providers page channel health uses `/v0/management/api-key-usage`; keep its response shape as `provider -> "base_url|api_key" -> success/failed/recent_requests`. The endpoint should restore counts and 20-bucket status bars from persisted `usage_events` by live `auth_index` when available, then fall back to runtime `Auth.Success`/`Auth.Failed` and in-memory recent buckets when no persisted rows exist.
