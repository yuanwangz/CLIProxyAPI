...
func effectiveAuthFileStatus(auth *coreauth.Auth, now time.Time) (coreauth.Status, string, bool, time.Time, int) {
	...
	status, message := auth.Status, auth.StatusMessage
	unavailable, nextRetryAfter := auth.Unavailable, auth.NextRetryAfter
	statusCode := 0
	if auth.LastError != nil {
		statusCode = auth.LastError.HTTPStatus
	}
	if auth.Archived || status == coreauth.StatusArchived {
		return coreauth.StatusArchived, message, unavailable, nextRetryAfter, statusCode
	}
	if auth.Disabled || status == coreauth.StatusDisabled {
		return status, message, unavailable, nextRetryAfter, statusCode
	}
	if unavailable && !nextRetryAfter.IsZero() && !nextRetryAfter.After(now) {
		...
		if status == coreauth.StatusError {
			status, message = coreauth.StatusActive, ""
		}
	}
	return status, message, unavailable, nextRetryAfter, statusCode
}
