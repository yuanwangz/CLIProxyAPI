...
func (h *Handler) RefreshAuthFile(c *gin.Context) {
	...
	targetAuth, _ := h.lookupAuthFile(name, strings.TrimSpace(req.AuthIndex))
	if targetAuth == nil {
...
func (h *Handler) PatchAuthFileStatus(c *gin.Context) {
	...
	if coreauth.IsPluginVirtualAuth(targetAuth) {
		if req.Disabled == nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": "archived is not supported for plugin virtual auth"})
			return
		}
...
func syncAuthFileDisabledState(auth *coreauth.Auth) {
	...
	// XX：管理字段路径与显式状态接口保持一致；归档凭证重新启用时仍保留归档状态，
	// 同时清理陈旧可用性/错误状态，避免被错误恢复为 active。
	applyAuthDisabledState(auth, disabled)
}
...
func syncAuthFileArchivedState(auth *coreauth.Auth) {
	...
	} else if !auth.Disabled && auth.Status == coreauth.StatusArchived {
		auth.Status = coreauth.StatusActive
	}
}
