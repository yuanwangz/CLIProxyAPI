...
func TestPatchAuthFileFields_ReenableArchivedAuthKeepsArchivedStatus(t *testing.T) {
	t.Setenv("MANAGEMENT_PASSWORD", "")
	...
	ctx.Request = httptest.NewRequest(
		...
		strings.NewReader(`{"name":"archived.json","disabled":false}`),
	)
	h.PatchAuthFileFields(ctx)
	...
	if !ok || updated == nil {
		t.Fatal("expected auth record to remain registered")
	}
	if updated.Disabled {
		t.Fatal("Disabled = true, want false")
	}
	if !updated.Archived || updated.Status != coreauth.StatusArchived {
		t.Fatalf("archived state = archived:%v status:%q, want archived:true status:%q", updated.Archived, updated.Status, coreauth.StatusArchived)
	}
	if updated.LastError != nil {
		t.Fatalf("LastError = %#v, want nil after operator re-enable", updated.LastError)
	}
}
