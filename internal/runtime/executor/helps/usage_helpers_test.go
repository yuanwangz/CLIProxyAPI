...
func TestUsageReporterBuildAdditionalModelRecordSkipsZeroTokens(t *testing.T) {
	...
	if _, ok := reporter.buildAdditionalModelRecord("gpt-image-2", usage.Detail{}, false); ok {
		t.Fatalf("expected all-zero token usage to be skipped")
	}
	if _, ok := reporter.buildAdditionalModelRecord("gpt-image-2", usage.Detail{}, true); !ok {
		t.Fatalf("expected explicit empty usage event to be recorded")
	}
	if _, ok := reporter.buildAdditionalModelRecord("gpt-image-2", usage.Detail{InputTokens: 2}, false); !ok {
		t.Fatalf("expected non-zero input token usage to be recorded")
	}
	if _, ok := reporter.buildAdditionalModelRecord("gpt-image-2", usage.Detail{CachedTokens: 2}, false); !ok {
