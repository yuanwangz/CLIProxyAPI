...
func TestGitTokenStoreSaveRetryAfterLeaseConflictCommitsMatchingContent(t *testing.T) {
	...
	assertRemoteAuthFileContents(t, remoteDir, "master", "auths/local.json", "local")
	assertRemoteTreePath(t, remoteDir, "master", "auths/concurrent.json", true)
}
...
func TestEnsureRepositoryRetryRestoresTrackedAuthOnUpToDatePull(t *testing.T) {
	...
	assertLocalAuthFileContents(t, authPath, "remote")
...
func TestGitTokenStoreCorruptionRecoveryUsesLatestRemoteAuthTree(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.wantExists {
				assertRemoteAuthFileContents(t, remoteDir, "master", "auths/victim.json", "remote-new")
			}
		})
	}
}
...
func TestGitTokenStoreCorruptionRecoveryPreservesOnlyNonConflictingLocalChanges(t *testing.T) {
	...
	t.Run("same-path conflict", func(t *testing.T) {
		...
		assertRemoteAuthFileContents(t, owner.remote, "master", "auths/victim.json", "remote-new")
	})
}
...
func TestGitTokenStoreFullPackfileCorruptionFailsClosedWithDirtyManagedFile(t *testing.T) {
	...
	t.Run("auth", func(t *testing.T) {
		...
		assertRemoteAuthFileContents(t, remoteDir, "master", "auths/dirty.json", "remote")
...
func assertLocalAuthFileContents(t *testing.T, path, wantAccessToken string) {
	t.Helper()

	contents, errRead := os.ReadFile(path)
	if errRead != nil {
		t.Fatalf("read local auth file %s: %v", path, errRead)
	}
	assertAuthFileJSON(t, string(contents), path, wantAccessToken)
}

func assertAuthFileJSON(t *testing.T, contents, path, wantAccessToken string) {
	t.Helper()

	metadata := make(map[string]any)
	if errUnmarshal := json.Unmarshal([]byte(contents), &metadata); errUnmarshal != nil {
		t.Fatalf("unmarshal auth file %s: %v", path, errUnmarshal)
	}
	if got, _ := metadata["access_token"].(string); got != wantAccessToken {
		t.Fatalf("auth file %s access_token = %q, want %q", path, got, wantAccessToken)
	}
	if got, _ := metadata["type"].(string); got != "codex" {
		t.Fatalf("auth file %s type = %q, want codex", path, got)
	}
	if got, _ := metadata["disabled"].(bool); got {
		t.Fatalf("auth file %s disabled = true, want false", path)
	}
	if got, _ := metadata["archived"].(bool); got {
		t.Fatalf("auth file %s archived = true, want false", path)
	}
	createdAt, _ := metadata["credential_created_at"].(string)
	if createdAt == "" {
		t.Fatalf("auth file %s credential_created_at is empty", path)
	}
	if _, errParse := time.Parse(time.RFC3339Nano, createdAt); errParse != nil {
		t.Fatalf("auth file %s credential_created_at = %q, want RFC3339 timestamp: %v", path, createdAt, errParse)
	}
}
...
func assertRemoteFileContents(t *testing.T, remoteDir, branch, path, wantContents string) {
	...
	contents := readRemoteFileContents(t, remoteDir, branch, path)
	if contents != wantContents {
		t.Fatalf("remote file %s contents = %q, want %q", path, contents, wantContents)
	}
}

func assertRemoteAuthFileContents(t *testing.T, remoteDir, branch, path, wantAccessToken string) {
	t.Helper()

	assertAuthFileJSON(t, readRemoteFileContents(t, remoteDir, branch, path), path, wantAccessToken)
}

func readRemoteFileContents(t *testing.T, remoteDir, branch, path string) string {
	t.Helper()
