...
func (m *Manager) pickNextLegacy(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, error) {
	...
	for _, candidate := range m.auths {
		if candidate == nil || executorKeyFromAuth(candidate) != provider || candidate.Archived || candidate.Status == StatusArchived || candidate.Disabled {
			continue
		}
...
func (m *Manager) refreshAuthForRequest(ctx context.Context, id, failedAccessToken string) (*Auth, error) {
	...
	if enableAfterRefresh {
		modelsToResume = clearUnauthorizedDisabledStateAfterRefresh(updated, now)
	}
