...
func (m *Manager) MarkResult(ctx context.Context, result Result) {
	...
	if auth, ok := m.auths[result.AuthID]; ok && auth != nil {
		...
		if result.Success && (auth.Disabled || auth.Status == StatusDisabled) {
			...
			if modelKey != "" {
				if m.probeMode || !shouldSkipCredentialCooldown(result.Error) {
					...
					if isModelSupportResultError(result.Error) {
						...
						case 401:
							_ = ClearConsecutiveStatusFailures(auth)
							durableUnauthorized := isDurableUnauthorizedResultError(result.Error)
							if durableUnauthorized || disableCooling {
								state.NextRetryAfter = time.Time{}
							} else {
								state.NextRetryAfter = now.Add(30 * time.Minute)
							...
							state.Quota = QuotaState{}
							if !preserveUnauthorized && durableUnauthorized {
								applyUnauthorizedDisabledState(auth, result.Error, now)
							} else if !preserveUnauthorized && !disableCooling {
								auth.Unavailable = true
							...
							if result.CredentialScope && !disableCooling {
								for _, otherState := range auth.ModelStates {
									if otherState != nil && otherState != state {
										...
										otherState.Quota = QuotaState{Exceeded: true, Reason: "credential_quota", NextRecoverAt: otherNext, BackoffLevel: backoffLevel}
									}
								}
								auth.Unavailable = true
...
func applyAuthFailureState(auth *Auth, resultErr *Error, retryAfter *time.Duration, now time.Time, disableCooling bool) {
	...
	case 401:
		_ = ClearConsecutiveStatusFailures(auth)
		if isDurableUnauthorizedResultError(resultErr) {
