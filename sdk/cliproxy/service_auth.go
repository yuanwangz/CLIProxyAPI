...
func (s *Service) resolveCooldownStateStore(cfg *config.Config) coreauth.CooldownStateStore {
	// This fork persists cooldown state in usage.sqlite through internal/usagepersist.
	// Keep upstream's file/PostgreSQL cooldown store path disabled to avoid duplicate restore and persistence.
	_ = cfg
	return nil
}
